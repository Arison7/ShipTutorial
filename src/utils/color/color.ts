export class Color {
	...
	constructor( r : number, g : number, b : number, a : number = 1 ){
		if(!Color.IsValidChannel(r)){
			throw new Error(`Invalid color channel value for R: ${r}`)
		}
		if(!Color.IsValidChannel(g)){
			throw new Error(`Invalid color channel value for G: ${g}`)
		}
		if(!Color.IsValidChannel(b)){
			throw new Error(`Invalid color channel value for B: ${b}`)
		}
		if(!Color.IsValidChannel(a, true)){
			throw new Error(`Invalid color channel value for A: ${a}`)
		}
		this.R = r
		this.G = g
		this.B = b
